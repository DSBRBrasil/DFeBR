...
namespace DFeBR.EmissorNFe.Danfe
{
    public class DanfeNFCeHtml : IDanfeHtml
    {
        ...
        private string MontarRodape(DanfeNFCe entidade)
        {
            ...
            var dadosProtocolo = "";
            if (!string.IsNullOrWhiteSpace(entidade.Protocolo))
                dadosProtocolo = $"Protocolo de Autorização: {entidade.Protocolo} {entidade.DataEmissao:dd/MM/yyyy HH:mm:ss}";
        ...
        private string MontarDestinatario(DanfeNFCe entidade)
        {
            ...
            list.Add("#Data#", doc);
            var endereco = "";
            if (destinatario.Endereco != null) endereco = $"{destinatario.Endereco.NomeEnd} {destinatario.Endereco.Cidade}";
        ...
        {
            return "";
            //if (entidade.DadosTributos == null) return "";
            //var list = new Dictionary<string, string>();
            //var str = Regex
            ...
            //list.Add("#Informations#", entidade.DadosTributos);
        ...
        private string MontarPagamentos(DanfeNFCe entidade)
        {
            ...
            {
                if (entidade.Pagamento.Troco == 0)
                {
                    list.Add("#StyleTroco#", "style=\"display: none;\"");
                }
                else
                {
                    //Exibir informacao de troco
                    var valorTroco = entidade.Pagamento.Troco.Value.FormatarNumeroDanfe();
                    list.Add("#ValorTroco#", valorTroco);
                } 
            }  
