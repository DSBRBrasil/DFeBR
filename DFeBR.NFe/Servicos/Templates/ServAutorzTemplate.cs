...
{
    /// <summary>
    /// Template de Autorização de NFe
    /// </summary>
    public abstract class ServAutorzTemplate : IServAutorzTemplate
    {
        ...
        private string ObterUrlContingencia(NFe entity)
        {
            ...
            switch (tpEmissao)
            {
                case TipoEmissao.ContingenciaDpec:
                    return _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFe.Producao.ContingenciaDpec;
                case TipoEmissao.ContingenciaSvcAn:
                    return _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFe.Producao.ContingenciaSvcan;
                case TipoEmissao.ContingenciaSvcRs:
                    return _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFe.Producao.ContingenciaSvcrs;
                default:
        ...
        ///     <para>
        ///         O tipo selecionado na tag NFe.infNFe.ide.tpEmis direciona para qual url deve ser direcionado o serviço, se
        ///         normal ou em contingencia
        ///     </para>
        /// </summary>
        /// <returns></returns>
            ...
            switch (entity.infNFe.ide.mod)
            {
                case ModeloDocumento.NFe:
                    var url = _emisorEmissorServicoConfig.Ambiente == TipoAmbiente.Homologacao
                            ? _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFe.Homologacao.Autoriza
                            : _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFe.Producao.Autoriza;
                    return url;
                case ModeloDocumento.NfCe:
                    var url1 = _emisorEmissorServicoConfig.Ambiente == TipoAmbiente.Homologacao
                            ? _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFce.Homologacao.Autoriza
                            : _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFce.Producao.Autoriza;
                    return url1;
                default:
        ...
        private IRetAutorz ProcessarEnvioMensagem(enviNFe d1)
        {
            ...
            ws.Wait();
            var strmensagem = ws.Result;
            var resposta = Utils.ObterNodeDeStringXml("nfeResultMsg", strmensagem);
        ...
        private RetAutorz RetornoProcessamento(string xmlRecebido, enviNFe enviNFe, bool contingencia)
        {
            ...
            if (retorno1.protNFe != null)
                if (StatusSefaz.ListarCodigo.All(n => retorno1.protNFe.infProt.All(m => m.cStat != n.Key)))
                    _rejeitadas++;
            var xmlEnviado = Utils.ClasseParaXmlString(enviNFe);
            var retorno2 = new RetAutorz(retorno1, xmlRecebido, _processadas, _rejeitadas, xmlEnviado, contingencia, _urlServico,
                    _urlWsdlServico, NomeServico);
        ...
        {
            var url = _emisorEmissorServicoConfig.Ambiente == TipoAmbiente.Homologacao
                    ? _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFce.Homologacao.QrcodeConsulta
                    : _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFce.Producao.QrcodeConsulta;
            return url;
        }
        ...
        {
            var url = _emisorEmissorServicoConfig.Ambiente == TipoAmbiente.Homologacao
                    ? _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFce.Homologacao.QrcodeChave
                    : _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsNFce.Producao.QrcodeChave;
            return url;
        }
        ...
        private void SalvarArquivoLoteEnviado(string resposta)
        {
            ...
            var nomeArq = $"lote_{_idlote}_{DateTime.Now:yyyy-MM-dd_HH-mm-ss-fff}-env-lot.xml";
        ...
        private void SalvarArquivoLoteRecebidos(RetAutorz retornoNfeAutorizacao)
        {
            ...
            if (string.IsNullOrWhiteSpace(_emisorEmissorServicoConfig.DiretorioArquivoRetornoXml))
                throw new InvalidOperationException("Informe um diretório válido.");
            var nomeArq = $"lote_{_idlote}_{DateTime.Now:yyyy-MM-dd_HH-mm-ss-fff}-rec.xml";
        ...
        private ICollection<string> ObterListaNomeSchemas()
        {
            ...
            var arqs = _emisorEmissorServicoConfig.ConfigEnderecoServico.Schemas.Autoriza.ToList();
        ...
        {
            return _emisorEmissorServicoConfig.ConfigEnderecoServico.UrlsWsdl.Autoriza;
        }
